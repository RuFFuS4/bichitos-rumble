// ---------------------------------------------------------------------------
// Network events + shared surface — SIN dependencia runtime de Colyseus
// ---------------------------------------------------------------------------
//
// H2 slice 4 (2026-08-18): este módulo existe para que el chunk de
// @colyseus/sdk (~35 kB gz) salga del grafo EAGER. Todo lo que game.ts,
// online-identity y los toasts necesitan estáticamente (interfaces de
// eventos, wrappers room.onMessage, sendInput, getDefaultServerUrl y el
// TIPO Room) vive aquí con `import type` del SDK — se borra en compile,
// cero bytes de Colyseus en el arranque. El código que SÍ instancia el
// SDK (Client, Callbacks) queda en network.ts, que solo se importa
// dinámicamente al pulsar "Online Multiplayer".
// ---------------------------------------------------------------------------
...
/** Firma de network.onPlayersChange — se pasa por parámetro desde el
 *  import dinámico para que este módulo no toque el SDK en runtime. */
export type PlayersChangeBinder = (
  room: Room,
  handlers: {
