...
|-----------|-----|---|---|----------|------------------------|
| **Sergei** | Balanced | `Gorilla Rush` — dash frontal con burst de masa/velocidad; golpea una vez a cada rival que toca (`dashHitForce` 22, un palmetazo: la víctima sale a ≤ 19,6 u/s, bajo el tope de 20 que alcanza todo cabezazo; 2026-09-24) | `Shockwave` — onda expansiva radial (force 68) | `Frenzy` — buff velocidad + masa; casi inamovible: mass 5.50 y todo empujón que recibe × 0,4 (`knockbackTakenMult`, 2026-09-24: golpes, habilidades y bolas; el Grip lo trae entero (Rafa, 2026-09-25) y el All-in lo echa) | Ninguno — alineado con diseño. |
| **Trunk** | Bruiser | `Trunk Ram` — dash pesado que arrolla (massMult máximo del roster) | `Trunk Slam` — pisotón AoE radius 7 + stun 1.5 s; stunned recibe ×4 knockback (Slam → cabezazo elimina) | `Trunk Grip` — yank frontal range 28 / cono 35° + stun 2.5 s + ×4 vulnerable (era 3.8 s; desde 2026-09-24 el aturdido, de Grip o de Slam, tampoco deja cabecear ni lanzar J, K o L, y suelta la carga del All-in). La víctima llega a la trompa en un paso, pero su modelo se desliza hasta ella en 0,15 s (`FEEL.grip.yankVisualTime`, 2026-09-24) | Grab-and-throw no existe (Grip acerca y stunnea, no lanza). Stampede RETIRADA. |
| **Kurama** | Trickster | `Fox Dash` — dash ligero que atraviesa a los rivales: mientras dura no choca ni empuja (`dashPhaseThrough`, 2026-09-24) | `Mirror Trick` — decoy clon en el sitio (opaco, con contorno, se funde en el 30 % final) + Kurama casi invisible (alpha 0.08; hasta 2026-09-24 el parpadeo de inmunidad lo tapaba y nunca se vio) + escape 7 u hacia atrás y lejos del perseguidor: el enemigo vivo más cercano a menos de 10 u (`decoyThreatRange`; se gira hacia él y salta de espaldas; sin nadie tan cerca, atrás de su facing; 2026-09-24), a suelo vivo (si no, al 70 % o al 40 % de la línea, o se queda junto al señuelo; 2026-09-24); los bots le pierden el rastro. La inmunidad del truco la libra de empujones, no del vacío: si el suelo se hunde bajo ella o se sale del disco, cae (2026-09-24) | `Copycat` — copia la L del último enemigo golpeado (`lastHitTargetCritter` synced; los campos de `COPYCAT_KEYS`, solo para ese lanzamiento, y el objetivo se consume) + chip HUD del target; sin target = solo el buff | Copiar a Sebastian da solo el buff: el All-in copiado se resolvía desde donde estuviera Kurama y la echaba o eliminaba seguro (2026-09-24). La versión segura de su L la decide Rafa. |
| **Shelly** | Tank | `Shell Charge` — dash rodando que golpea (`dashHitForce` 30, 2026-09-24) | `Steel Shell` — 4 s rooted + inmune + anclada (mass ×9999); el atacante rebota. Al pulsar K frena en seco lo que se mueve ella: corta la J en curso y no desliza en la carga ni al cerrarse. Un empujón que ya lleva, o que le llega en la carga, sigue su curso (`anchorBrakeMaxSpeed` 3,5 u/s: por encima de su carrera, más la J y la estocada, es un empujón). La inmunidad la libra de empujones, no del vacío: si la baldosa se hunde bajo el caparazón, cae (2026-09-24) | `Saw Shell` — spin del caparazón a 22 rad/s; todo contacto expulsa con impulso 90 | Sin damage-reflect ni Mega Shell rodante; cabeza/patas no se ocultan (limitación GLB). |
...
| **Sihans** | Trapper | `Burrow Rush` — dash excavador | `Burrow` — blink 6.5 u con invisibilidad total 0.3 s + zona quicksand slow en el origen; si el destino no es suelo vivo, retrocede por la línea hasta encontrarlo o se queda en el sitio (2026-09-24) | `Sinkhole` — agujero REAL: mata los fragmentos del arena bajo el disco (`arenaFragmentsKilled`) salvo el que pisa Sihans (2026-09-24; online falta en `BrawlRoom`) + pull hacia el centro; centro del mapa protegido | Tunnel solo deja zona en el origen, no en la salida. |
| **Kowalski** | Mage | `Ice Slide` — dash de panza que desliza: su rozamiento va ×3 mientras dura (`slideFrictionMult`), unas 4,7 u más que correr en vez de 1,6 (2026-09-24) | `Snowball` — proyectil real server-authoritative; knockback + slow 50 % / 5 s; la lanza con la aleta: clip a 3,5× para que la suelte en el fotograma en que nace la bola (2026-09-24); al impactar deja un anillo de nieve, con hit stop corto, en vez de polvo beige (2026-09-24) | `Frozen Floor` — zona resbaladiza radius 8 / 7 s (friction ×5, accel ×0.35: `floorFrictionMult` / `floorAccelMult` en la definición; offline la aceleración no se reducía hasta 2026-09-24); owner inmune; buff ligero ×1,10 de velocidad y masa durante los 3 s de la L | La bola vuela a media altura y no se desliza por el suelo como decía la ficha: Rafa la quiere lanzada con la aleta (2026-09-24). Ice Age shipped como Frozen Floor. |
| **Cheeto** | Assassin | `Pounce` — salto felino que golpea (`dashHitForce` 30, 2026-09-24) | `Shadow Step` — blink al enemigo más cercano (seek 9 u) + golpe radial al aterrizar | `Cone Pulse` — channel rooted 1.8 s; 6 ondas frontales que se expanden con fuerza doblando `min(2^(N-1), 8)`; empuje hacia delante, no radial. Al arrancar pinta en el suelo la cuña que va a barrer (±45°, 9,4 u) en vez del anillo de 360° de las demás L (2026-09-24) | Ninguno — Tiger Roar shipped como Cone Pulse frontal. |
| **Sebastian** | Glass Cannon | `Claw Rush` — dash con pinza; golpea (`dashHitForce` 26) solo a quien tiene delante, ±60° de la mirada como su Claw Wave (`dashHitArcDeg`, 2026-09-24) | `Claw Wave` — pound frontal cónico ±60° (force 76) con VFX half-disc | `All-in` — hold-to-fire: rooted + preview de trayectoria en el suelo; mientras carga apunta con el stick (el facing gira hacia el mando a 180°/s, `FEEL.allIn.aimTurnDegPerSec`, y la línea lo sigue; 360 → 180 el 2026-09-25, para que la línea avise a tiempo); release = dash frontal por donde apunta, nunca antes de 0,35 s de carga (`holdToFireMinMs`: un toque sale a los 0,35 s; 2026-09-24); hit = yeet garantizado al void (solo objetivos por delante; pasillo en `FEEL.allIn.hitMargin`), miss = Sebastian cae al void en el primer punto de su línea fuera de la arena, aunque sea inmune (2026-09-24) | Diseño pedía dash LATERAL; la versión final es frontal-only (decisión del BLOQUE FINAL, no un pendiente). |

**Brillo de estado (2026-09-24).** El color emisivo mientras dura un
cabezazo o una habilidad vive en `FEEL.stateGlow`. La carga de la K
sigue en amarillo («apártate») y la de la L pasa a un latido rojo oscuro;
antes las dos eran el mismo amarillo. Cada definición puede traer los
suyos (`activeGlowHex`, `activeGlowIntensity`, `windUpGlowHex`):
- Kowalski: Ice Slide en hielo (0x9fe3ff), la carga de la bola en azul
  (0x88c1ff) y Frozen Floor en escarcha (0xcfeeff).
- Saw Shell en verde (0x6ddfa9).
- Sinkhole en arena, suave (0xc89a3c, pico 0,4): la L es el agujero, no
  Sihans.
- Toxic Touch conserva su parpadeo hipnótico morado y rosa.

Copycat no copia el brillo: la Kurama que copia late en rojo.
...
- *(2026-09-24: esos eran los clips de entonces. Hoy el kit usa `Ability1` 1,20 s a 2,3× (`clipPlaybackRate`), `Ability2` 5,50 s a 1,5× y `Ability3` 2,67 s a 2× (velocidades de `ANIMATION_OVERRIDES`). Hasta esa fecha las velocidades de los overrides no llegaban al juego y la K y la L corrían a 1×; ver `ABILITY_QA_CHECKLIST.md`, grupo G7.)*
