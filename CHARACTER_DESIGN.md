...
| **Kurama** | Trickster | `Fox Dash` — dash ligero | `Mirror Trick` — decoy clon en el sitio (opaco, con contorno, se funde en el 30 % final) + Kurama casi invisible (alpha 0.08; hasta 2026-09-24 el parpadeo de inmunidad lo tapaba y nunca se vio) + escape 7 u atrás, a suelo vivo (si no, al 70 % o al 40 % de la línea, o se queda junto al señuelo; 2026-09-24); los bots le pierden el rastro | `Copycat` — copia la L del último enemigo golpeado (`lastHitTargetCritter` synced; los campos de `COPYCAT_KEYS`, solo para ese lanzamiento, y el objetivo se consume) + chip HUD del target; sin target = solo el buff | Copiar a Sebastian da solo el buff: el All-in copiado se resolvía desde donde estuviera Kurama y la echaba o eliminaba seguro (2026-09-24). La versión segura de su L la decide Rafa. |
...
| **Kowalski** | Mage | `Ice Slide` — dash de panza | `Snowball` — proyectil real server-authoritative; knockback + slow 50 % / 5 s | `Frozen Floor` — zona resbaladiza radius 8 / 7 s (friction ×5, accel ×0.35: `floorFrictionMult` / `floorAccelMult` en la definición; offline la aceleración no se reducía hasta 2026-09-24); owner inmune | Ninguno — Ice Age shipped como Frozen Floor. |
