...
| Colyseus | ✅ 2026-08-18: **0.16 → 0.17 + schema v4, lockstep** (server colyseus 0.17.10 / cliente @colyseus/sdk 0.17.43). El adapter previo (fase A) redujo el cambio cliente a `network.ts`. Hallazgos del proceso: (1) el wiring HTTP clásico duplica listeners en 0.17 → `ERR_HTTP_HEADERS_SENT` en /api — migrado a `defineServer()` con api.ts intacto como middleware (express 5 llega transitivo); (2) la review adversarial cazó la **auto-reconexión default-ON del SDK** (partida congelada ~1 min en desconexión brusca sin allowReconnection server-side) → desactivada en `connectToBrawl`, reactivar en H4. Verificado con e2e local de 2 páginas (misma sala, nicknames v4, fases, movimiento de bots) + review de 3 lentes. |
| three.js | ✅ 2026-08-17: **0.172 → 0.185.1** (+ @types 0.185.4). Fallout de tipos: CERO. Imports `three/examples/jsm` → `three/addons` (2 líneas, model-loader). Pase visual headless (Playwright + capturas): selector, match, skybox, decor packs y belts 3D correctos — el cambio de especular r181 no afecta (metalness 0 forzado en crítters; los belts hasta ganan). Nota: r185 ya no exporta `three/package.json`. |
| Vite | ✅ 2026-08-17: **6.4 → 7.3 → 8.2 (Rolldown)**. La 7 pasó sin tocar nada. La 8 construía ya en modo compat; config modernizada: `rollupOptions` → `rolldownOptions`, `manualChunks` → `output.codeSplitting.groups` (three/colyseus idénticos), `__dirname` → `import.meta.dirname`. Verificado: terser sigue stripeando console.log, DCE de Sentry intacta con y sin DSN, 5 entries HTML OK. Build 4,9 → **2,9 s**. Aviso nuevo de Rolldown anotado: `INEFFECTIVE_DYNAMIC_IMPORT` en online-identity (import estático+dinámico mezclados — limpieza para el split lazy de colyseus en H2). |
